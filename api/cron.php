	...
	$data = array(
		...
		'data'   => array(
			...
			'api'         => array(
				'version' => 4,
				'compat'  => 3,
				'bev'     => 'mnfl=' . AITNINJA_BEV . '.' . AITNINJA_CRONVERSION,
			),
		),
	);
	...
	if ($result = $mysqli->query($sql)) {
		...
		foreach ($blocks as $block) {
			...
			if (!array_key_exists($minerkey, $perminer)) {
				$perminer[$minerkey] = array(
					'PoolPubKeys'                  => array($block['BlockPoolPubKey']),
			...
			if (!array_key_exists($block['BlockMNProtocol'], $perversion)) {
				...
				$perversion[$block['BlockMNProtocol']] = array(
		...
		$data = array(
			...
			'data'   => array(
				...
				'api'    => array(
					'version' => 3,
					'compat'  => 1,
					'bev'     => 'bk24h=' . AITNINJA_BEV . "." . AITNINJA_CRONVERSION,
				),
			),
		);
	...
	$data = array(
		...
		'data'   => array(
			...
			'api'   => array(
				'version' => 3,
				'compat'  => 3,
				'bev'     => 'mnst=' . AITNINJA_BEV . '.' . AITNINJA_CRONVERSION,
			),
		),
	);
		...
		while (($row = $result->fetch_assoc()) && ($numblocks < 11)) {
			...
			if ($numblocks < 11) {
				...
				if (!array_key_exists($row['BlockMNPayee'], $bhinfo[$row['BlockHeight']][$row['Protocol']])) {
					$bhinfo[$row['BlockHeight']][$row['Protocol']][$row['BlockMNPayee']] = array(
						'count' => 0,
						'names' => array(),
					);
				}
	...
	$data = array(
		...
		'data'   => array(
			...
			'api'             => array(
				'version' => 3,
				'compat'  => 3,
				'bev'     => 'bkcs=' . AITNINJA_BEV . '.' . AITNINJA_CRONVERSION,
			),
		),
	);
...
function generate_governancevotelimit_json_files($mysqli, $testnet = 0) {
	...
	if ($result = $mysqli->query($sql)) {
		...
		$currentblock["BlockId"]         = isset($currentblock["BlockId"]) ? intval($currentblock["BlockId"]) : 0;
		$currentblock["BlockTime"]       = isset($currentblock["BlockTime"]) ? intval($currentblock["BlockTime"]) : 0;
		$currentblock["BlockDifficulty"] = isset($currentblock["BlockDifficulty"]) ? floatval($currentblock["BlockDifficulty"]) : 0;
	} else {
		echo "SQL error - " . $mysqli->errno . ": " . $mysqli->error . "\n";
	...
	$data = array(
		...
		'data'   => array(
			...
			'api'       => array(
				'version' => 1,
				'compat'  => 1,
				'bev'     => 'gpvl=' . AITNINJA_BEV . "." . AITNINJA_CRONVERSION,
			),
		),
	);
...
function generate_governanceproposals_json_files($mysqli, $testnet = 0) {
	...
	if ($testnet == 0) {
		$nextsuperblock = $currentblock["BlockId"] - ($currentblock["BlockId"] % 16616) + 16616;
		for ($i = 210240; $i <= $nextsuperblock; $i += 210240) {
			$nSubsidy -= $nSubsidy / 14;
		}
		$estimatedbudgetamount = (($nSubsidy / 100) * 10) * (60 * 24 * 30) / 2.6;
	} else {
		$nextsuperblock = $currentblock["BlockId"] - ($currentblock["BlockId"] % 50) + 50;
		for ($i = 46200; $i <= $nextsuperblock; $i += 210240) {
			$nSubsidy -= $nSubsidy / 14;
		}
	...
	echo "OK (" . $currentblock["BlockId"] . " / Estimated budget amount: " . $estimatedbudgetamount . " AIT)\n";
	...
	$data = array(
		...
		'data'   => array(
			...
			'api'                 => array(
				'version' => 2,
				'compat'  => 1,
				'bev'     => 'gp=' . AITNINJA_BEV . "." . AITNINJA_CRONVERSION,
			),
		),
	);
...
function generate_governancetriggers_json_files($mysqli, $testnet = 0) {
	...
	$sql = sprintf("SELECT cgot.GovernanceObjectId Hash, cgot.GovernanceObjectEventBlockHeight BlockHeight, " . " cgot.GovernanceObjectVotesAbsoluteYes AbsoluteYes, cgot.GovernanceObjectVotesYes Yes, " . " cgot.GovernanceObjectVotesNo No, cgot.GovernanceObjectVotesAbstain Abstain, " . " cgot.GovernanceObjectBlockchainValidity BlockchainValidity, " . " cgot.GovernanceObjectIsValidReason IsValidReason, " . " cgot.GovernanceObjectCachedValid CachedValid, " . " cgot.GovernanceObjectCachedFunding CachedFunding, " . " cgot.GovernanceObjectCachedDelete CachedDelete, " . " cgot.GovernanceObjectCachedEndorsed CachedEndorsed, " . " cgot.FirstReported FirstReported, " . " cgot.LastReported LastReported, " . " cgotp.GovernanceObjectPaymentPosition PaymentPosition, " . " cgotp.GovernanceObjectPaymentAddress PaymentAddress, " . " cgotp.GovernanceObjectPaymentAmount PaymentAmount, " . " cgotp.GovernanceObjectPaymentProposalHash PaymentProposalHash, " . " cgop.GovernanceObjectName PaymentProposalName " . "FROM cmd_gobject_triggers cgot " . "LEFT JOIN cmd_gobject_triggers_payments cgotp ON (cgotp.GovernanceObjectTestnet = cgot.GovernanceObjectTestnet AND cgotp.GovernanceObjectId = cgot.GovernanceObjectId) " . "LEFT JOIN cmd_gobject_proposals cgop ON (cgop.GovernanceObjectTestnet = cgot.GovernanceObjectTestnet AND cgotp.GovernanceObjectPaymentProposalHash = cgop.GovernanceObjectId) " . "WHERE " . " cgot.GovernanceObjectTestnet = %d AND cgot.GovernanceObjectVotesAbsoluteYes > 0" . " AND cgot.GovernanceObjectCachedValid = 1" . " AND cgot.GovernanceObjectEventBlockHeight >= %d", $testnet, $currentblock["BlockId"]);
	if ($result = $mysqli->query($sql)) {
		$triggersvalid = 0;
		$triggers      = array();
	...
	$data = array(
		...
		'data'   => array(
			...
			'api'                => array(
				'version' => 1,
				'compat'  => 1,
				'bev'     => 'gt=' . AITNINJA_BEV . "." . AITNINJA_CRONVERSION,
			),
		),
	);
...
function generate_blockssuperblocks_json_files($mysqli, $testnet = 0) {
	...
	$sql         = sprintf("SELECT cib.BlockId BlockId, cib.BlockHash BlockHash, cib.BlockPoolPubKey BlockPoolPubKey, cpp.PoolDescription PoolDescription, cib.BlockMNPayee SuperblockV1PaymentAddress, " . "cib.BlockTime BlockTime, cib.BlockDifficulty BlockDifficulty, cib.IsSuperblock SuperblockVersion, cib.BlockMNValue TotalAmount, cib.SuperBlockBudgetName SuperblockV1BudgetName, " . "cgop.GovernanceObjectName SuperblockV2ProposalName, cibs.GovernanceObjectPaymentProposalHash SuperblockV2ProposalHash, cibs.GovernanceObjectPaymentAmount SuperblockV2ProposalPaymentAmount, " . "cibs.GovernanceObjectPaymentAddress SuperblockV2PaymentAddress FROM cmd_info_blocks cib " . "LEFT JOIN cmd_pools_pubkey cpp ON cib.BlockPoolPubKey = cpp.PoolPubKey AND cib.BlockTestNet = cpp.PoolTestNet " . "LEFT JOIN cmd_info_blocks_superblockpayments cibs ON cib.BlockTestNet = cibs.BlockTestNet AND cib.BlockId = cibs.BlockId " . "LEFT JOIN cmd_gobject_proposals cgop ON cibs.GovernanceObjectPaymentProposalHash = cgop.GovernanceObjectId AND cibs.BlockTestNet = cgop.GovernanceObjectTestNet " . "WHERE cib.BlockTestNet = %d AND cib.IsSuperblock > 0 ORDER BY BlockId DESC", $testnet);
	$superblocks = array();
	if ($result = $mysqli->query($sql)) {
		while ($row = $result->fetch_assoc()) {
	...
	$data = array(
		...
		'data'   => array(
			...
			'api'         => array(
				'version' => 1,
				'compat'  => 1,
				'bev'     => 'sb=' . AITNINJA_BEV . "." . AITNINJA_CRONVERSION,
			),
		),
	);
...
xecho('AIT Ninja Front-End JSON Generator cron v' . AITNINJA_BEV . '.' . AITNINJA_CRONVERSION . "\n");
if ($argc != 3) {
